...
module Material
  ...
  # Evaluate BRDF for pair of incoming and outgoing directions. This method is
  # typically called by an integrator when the integrator obtained the incident
  ...
  # by an integrator when the integrator obtained the outgoing direction of the
  # emission by sampling a point on a light source.
  #
  ...
  # Evaluate specular reflection. This method is typically called by a
  # recursive ray tracer to follow the path of specular reflection.
  #
  ...
  # method is typically called in a path tracer to sample and evaluate the next
  # path segment. The methods decides which component of the material to sample
  # (diffuse, specular reflection or refraction, etc.), computes an incident
  # direction, and returns the BRDF value, the direction, and the probability
  # density stored in a ShadingSample.
  #
  ...
  # sample as input. This method is typically called in a bidirectional path
  # tracer to sample and evaluate the first light path segment. The methods
  # computes an outgoing direction, and returns the emission value, the
  # direction, and the probability density (all stored in a ShadingSample).
  #
  # @param hit_record HitRecord
  # @param sample [Float]
  # @return a ShadingSample
